...
public class Pedido {
	...
	public double valorFinal() {
	 ...
	 public void mesclarPedido(Pedido outroPedido) {
		try {
			if (this.quantProdutos + outroPedido.quantProdutos <= 10) {
				for (int i = 0; i < this.quantProdutos; i++){
					for(int a = 0; a < outroPedido.quantosProdutos; i++){		
						if (produtos[i].descricao == outroPedido[a]. ) {
							this.pedido[i].quantidade += outroPedido[a].quantiade;
						} else {
							pedido.append(outroPedido[a]);
						}
					}
				}
				return pedido;
			...
			System.out.println("A soma das duas listas de pedido ultaprassa o limite máx!");
		}
